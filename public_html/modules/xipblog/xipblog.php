...
class xipblog extends Module implements WidgetInterface
{
	...
	public $all_hooks = array("displayheader","ModuleRoutes","displayhome", "gSitemapAppendUrls");
	...
	public function __construct()
	{
		...
		if ( !$this->isRegisteredInHook('gSitemapAppendUrls') ) {
	...
	{
		$xipblogPostsInfo = self::getXipblogPostsInfo();
		$sitemapInfo = array();
		...
		$ssl = null;
		$relative_protocol = false;
		$base_url = self::getBaseLink($id_shop, $ssl, $relative_protocol).self::getLangLink($id_lang, null, $id_shop);
		$imageType = xipimagetypeclass::GetAllImageTypes();
		$imageType = end($imageType);

		if ( isset($xipblogPostsInfo) && !empty($xipblogPostsInfo) )
		{
			foreach($xipblogPostsInfo as $post_info)
			{
				$sitemapInfo[] = array(
					'link' => self::XipBlogPostLink(array(
						'id' => $post_info['id_xipposts']
					)),
					'page' => 'category', // Priority: 0.8
					'lastmod' => $post_info['post_modified'],
					'image' => array(
						'link' => $base_url . xipblog_img_uri . $imageType['name'] . '-' . $post_info['post_img'],
						'title_img' => xippostsclass::get_the_title($post_info['id_xipposts'])['post_title'],
						'caption' => xippostsclass::get_the_excerpt($post_info['id_xipposts'])['post_excerpt']
					)
				);
			}
		}
		...
		// $xipblog_links = array(
		// 	array(
		// 		'link' => 'https://www.herbobarcelona.com/blog/post/test1',
		// 		'page' => 'category',
		// 		'lastmod' => '2020-03-19 17:28:43',
		// 		'image' => array(
		// 			'link' => 'https://www.herbobarcelona.com/blog/post/test1.jpg',
	...
	public function getXipblogPostsInfo() {
        $sql = 'SELECT id_xipposts, post_modified, post_img ' 
